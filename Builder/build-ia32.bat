...
wget -O "%TMPDIR%\nwjs-windows.zip" https://dl.nwjs.io/v0.17.3/nwjs-v0.17.3-win-ia32.zip 2>NUL
echo -------- Extracting Windows binaries
7z x "%TMPDIR%\nwjs-windows.zip" -o"%BUILDDIR%" >NUL
ren "%BUILDDIR%\nwjs-v0.17.3-win-ia32" "ePubViewer-%CURVERSION%-windows"

echo ---- Downloading Linux binaries
wget -O "%TMPDIR%\nwjs-linux.tar.gz" https://dl.nwjs.io/v0.17.3/nwjs-v0.17.3-linux-ia32.tar.gz 2>NUL
echo -------- Extracting Linux binaries
7z x "%TMPDIR%\nwjs-linux.tar.gz" -o"%TMPDIR%" >NUL
7z x "%TMPDIR%\nwjs-v0.17.3-linux-ia32.tar" -o"%BUILDDIR%" >NUL
ren "%BUILDDIR%\nwjs-v0.17.3-linux-ia32" "ePubViewer-%CURVERSION%-linux"
